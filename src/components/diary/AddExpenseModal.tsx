...
import type { TransactionType, ExpenseFormData, Category } from '@/types'
...
export default function AddExpenseModal({ onClose, onSave, initialData, categories }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-lg bg-surface rounded-t-3xl border border-border border-b-0
        ...
        <div className="flex items-center justify-between px-5 py-2">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-primary/50 transition-colors text-text-secondary">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-5 pb-8 space-y-5">
          ...
          <div className="flex bg-primary/50 rounded-xl p-1">
            {(['expense', 'income'] as const).map(t => (
              <button key={t} onClick={() => { setType(t); setSelectedCategory('') }}
                className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                  type === t ? (t === 'expense' ? 'bg-expense text-white shadow-sm' : 'bg-income text-white shadow-sm') : 'text-text-secondary'
                }`}
              >
          ...
          <div>
            <div className="text-center mb-4">
              <p className="text-4xl font-bold font-mono text-text-primary tracking-tight">€ {amount || '0'}</p>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {['1','2','3','4','5','6','7','8','9','.','0','backspace'].map(key => (
                <button key={key} onClick={() => handleNumpad(key)}
                  className="h-14 flex items-center justify-center rounded-xl bg-primary/50 hover:bg-primary/70 active:scale-95 transition-all text-lg font-medium text-text-primary"
                >
                  {key === 'backspace' ? (
          ...
          {/* Category picker */}
          <div>
            <label className="text-xs font-semibold text-text-secondary uppercase tracking-wide mb-2 block">Categoria</label>
            <button onClick={() => setShowCategoryPicker(!showCategoryPicker)}
              className="w-full flex items-center justify-between bg-primary/50 rounded-xl px-4 py-3 border border-border hover:border-accent/30 transition-colors"
            >
              {selectedCat ? (
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: selectedCat.color }} />
                  <span className="text-sm text-text-primary">{selectedCat.name}</span>
                </div>
              ) : (
            ...
            {showCategoryPicker && (
              <div className="mt-2 bg-primary/50 rounded-xl border border-border overflow-hidden max-h-48 overflow-y-auto">
                {filteredCategories.map(cat => (
                  <button key={cat.id} onClick={() => { setSelectedCategory(cat.id); setShowCategoryPicker(false) }}
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-surface transition-colors ${selectedCategory === cat.id ? 'bg-surface' : ''}`}
                  >
